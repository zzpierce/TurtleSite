  ...
  <div>
    <div class="main">
      <div v-for="article in articles" class="main-row">
        <h4 @click="toDetail(article.id)">{{article.title}}</h4>
  ...
  export default {
    ...
    methods: {
      loadArticles: function() {
        var self = this;
        $.get(self.root + 'resources/article', function(data) {
          var articles = JSON.parse(data);
          for(let article of articles) {
            article['createTime'] = self.subTime(article['createTime']);
            article['updateTime'] = self.subTime(article['updateTime']);
